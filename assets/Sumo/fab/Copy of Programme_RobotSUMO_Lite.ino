...
#define CapteurLigneAvantGauche_Pin  A1
#define CapteurLigneAvantDroite_Pin  A2
...
const int valeurDeReference = 550; // seuil pour noir et blanc trouvé par test
double Temps_n;
double Temps_n_1 = 0;
bool temps = true;
...
{
  static int DerniereValeur = 0;
  int ValeurNEC = 0;

  if (irrecv.decode(&Valeur)) {

    // Ignorer la trame de répétition NEC
    if (Valeur.value == 0xFFFFFFFF) {
      irrecv.resume();
...
void Reorienter(int vitesse, char sens) {
  if (sens == 'D') {
    analogWrite(PWMG, vitesse/2);
    analogWrite(PWMD, vitesse);
  }
  if (sens == 'G') {
    analogWrite(PWMD, vitesse/2);
    analogWrite(PWMG, vitesse);
  }
}
...
int Etat_batterie(uint8_t pin_lecture_info_batt) {
  if (digitalRead(pin_lecture_info_batt)==0){
     return 0; // Retourne directement la valeur numérique avec 1 correspondant à une batterie inférieur à 6,7 V, et 0 correspondant à une batterie supérieur à 6,7 V
  }
  else{
    int adc = analogRead(AnalogueBatterie_Pin);
    float Vpin = adc * 5.0 / 1023.0;
    float Vbat = Vpin * 2.14;
    if (Vbat<5.5){
      return 1;
    }
    else{return 0;}
...
void loop() {
  ...
  while(Broken!= true){
    ...
    if(Etat_batterie(DigitalBatterie_Pin)==1){
      Avancer(0);
      Broken = true;
      Buzzer();
      delay(250);
